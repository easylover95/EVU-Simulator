...
function App() {
  ...
  const persistRemote = useCallback((operation: (client: SupabaseClient) => PromiseLike<unknown>) => {
    if (!isSupabaseConfigured) return;
    persistQuietly(
      getSupabaseClient().then((client) => (client ? operation(client) : undefined)),
    );
  }, []);
  ...
  const pushNotifications = useCallback((items: Omit<Notification, 'id'>[]) => {
    ...
    persistRemote((client) => client.from('notifications').insert(full));
  }, [persistRemote]);
  ...
  const persistCompany = useCallback((next: Company) => {
    ...
    setCompany(next);
    saveCompanyEconomy({ balance: next.balance, reputation: next.reputation, lastTick: next.tick, level: next.level });
    persistRemote((client) =>
      client
        .from('company')
        .update({
  ...
  const advanceOneTick = useCallback(() => {
    ...
    persistRemote((client) =>
      client.from('company').update({ tick: nextTick, updated_at: gameNowIso }).eq('id', prevCompany.id),
    );
    ...
    for (const id of activatedIds) {
      persistRemote((client) => client.from('assignments').update({ status: 'aktiv' }).eq('id', id));
    }
  }, [
  ...
  function handleRejectOrder(order: Order) {
    ...
    persistRemote((client) => client.from('orders').update({ status: 'abgelehnt' }).eq('id', order.id));
  }
  ...
  function handleAcquireLoco(
    ...
    persistRemote((client) => client.from('locomotives').insert(loco));
  ...
  function handleRecruit(listing: JobListing, withFleetTraining = false): boolean {
    ...
    persistRemote((client) => client.from('drivers').insert(person));
    const live = companyRef.current;
    if (live) persistCompany(grantAchievements(live));
    return true;
  }
